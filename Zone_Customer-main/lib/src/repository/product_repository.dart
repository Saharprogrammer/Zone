...
Future<Favorite> addFavorite(Favorite favorite) async {
  ...
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}favorites?$_apiToken');
  try {
    final client = new http.Client();
...
Future<Favorite> removeFavorite(Favorite favorite) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}favorites/${favorite.id}?$_apiToken');
  try {
    final client = new http.Client();
...
Future<Review> addProductReview(Review review, Product product) async {
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}product_reviews');
  final client = new http.Client();
