...
Future<Order> addOrder(Order order, Payment payment) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}orders?$_apiToken');
  final client = new http.Client();
...
Future<Order> cancelOrder(Order order) async {
  ...
  User _user = userRepo.currentUser.value;
  final String _apiToken = 'api_token=${_user.apiToken}';
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}orders/${order.id}?$_apiToken');
  final client = new http.Client();
