...
Future<userModel.User> login(userModel.User user) async {
  final Uri url =  Uri.parse('${GlobalConfiguration().getValue('api_base_url')}login');
...
Future<userModel.User> register(userModel.User user) async {
  final Uri url =  Uri.parse('${GlobalConfiguration().getValue('api_base_url')}register');
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
...
Future<bool> resetPassword(userModel.User user) async {
  final Uri url =  Uri.parse('${GlobalConfiguration().getValue('api_base_url')}send_reset_link_email');
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
...
Future<userModel.User> update(userModel.User user) async {
  ...
  final Uri url =  Uri.parse('${GlobalConfiguration().getValue('api_base_url')}users/${currentUser.value.id}?$_apiToken');
  final client = new http.Client();
  print(user.toMap());
  final response = await client.post(
    url,
...
Future<Address> addAddress(Address address) async {
  ...
  final Uri url =  Uri.parse('${GlobalConfiguration().getValue('api_base_url')}delivery_addresses?$_apiToken');
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
...
Future<Address> updateAddress(Address address) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  address.userId = _user.id;
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}delivery_addresses/${address.id}?$_apiToken');
  final client = new http.Client();
