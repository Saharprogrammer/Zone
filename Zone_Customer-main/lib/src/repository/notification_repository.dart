...
Future<Notification> markAsReadNotifications(Notification notification) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}notifications/${notification.id}?$_apiToken');
  final client = new http.Client();
...
Future<Notification> removeNotification(Notification cart) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final Uri url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}notifications/${cart.id}?$_apiToken');
  final client = new http.Client();
